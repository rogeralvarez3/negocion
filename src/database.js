...
const tablas = function() {
  var result = new Promise(resolve => {
    cn.query(
      "select table_name from information_schema.tables where table_schema='negocion'",
      (err, rows) => {
        if (err) {
          resolve(err);
        } else {
...
const db = async function() {
  var result = await tablas().then(res => {
    if (res.errno) return [];
    var sql = [];
    res.forEach(t => {
      sql.push("select * from `" + t + "`");
    });
    sql = sql.join(";");
            ...
            cols[i].forEach(col => {
              myCols.push({
                ...
                text:
                  ...
                  col.name.slice(1, col.name.length).toLowerCase(),
                type: col.type,
                length: col.length,
                flag: col.flag
              });
...
const login = function(param) {
  var result = new Promise(resolve => {
    var sql =
      "select * from empleados where seudónimo ='" +
      param.u +
      "' and contraseña = '" +
      param.p +
      "'";
